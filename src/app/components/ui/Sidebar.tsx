...
const Sidebar: React.FC<SidebarProps> = ({ currentSection, setCurrentSection, navItems }) => (
  <nav className="fixed left-0 top-20 h-full w-64 bg-slate-900/90 backdrop-blur-lg border-r border-cyan-400/20 p-6 flex flex-col pt-6 space-y-4 overflow-y-auto">
  {navItems.map((item) => (
    <button
      key={item.id}
